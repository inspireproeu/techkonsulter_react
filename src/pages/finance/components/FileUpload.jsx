...
const FileUploadDialog = ({
  ...
  const uploadFile = async () => {
    ...
    formData.append('title', `Projects finance- ${importFiles[0].name}`);
      ...
      try {
        ...
        for (let i = 0; i <= filterassetIDRows.length; i++) {
          if (assetIDRows.length > 0 && filterassetIDRows[i] && filterassetIDRows[i].id) {
            ...
            if (project_id) {
              filterassetIDRows[i].project_id = project_id;
            }
            updateSuccess = await fetchPut(
              `${DATAURLS.PROJECTFINANCE.url}/${filterassetIDRows[i].id}`,
              filterassetIDRows[i],
              getAccessToken()
            )
        ...
        if (updateSuccess?.data?.id) {
          await uploadFile();
          let allprojectids = filterassetIDRows.map((row) => row.project_id);
          allprojectids = _.uniq(allprojectids)
          if (allprojectids.toString()) {
            await fetchGet(
  ...
  const handleInsert = async () => {
    ...
    try {
      ...
      if (insertSuccess && insertSuccess?.data.length > 0) {
        await uploadFile();
        let allprojectids = tempassetIDRows.map((row) => row.project_id);
        allprojectids = _.uniq(allprojectids)
        if (allprojectids.toString()) {
          await fetchGet(
            `${DATAURLS.UPDATEPROJECTFINANCE.url}?ids=${allprojectids.toString()}`,
    ...
    try {
      reader.onload = (e) => {
        ...
        if (project_id && data.length > 0) {
          ...
          const result = data.filter(item => Number(item.project_id) !== Number(project_id));
          if (result?.length > 0) {
            setSuccess(true);
            setFileUploadError(true);
            setFileUploadSuccess(false);
            setFileUploadErrorMessage(`${PROJECT_MISMATCH} - ${project_id}`);
            setLoading(false);
            return;
          }
        }
      ...
      <div className={classes.dialogRoot}>
        <DialogTitle id='alert-dialog-title' className={classes.dialogTitle}>
          <div className={classes.dialogTitleText}>
            <div>{title}{project_id ? " - " + project_id : ''}</div>
          </div>
          <CloseCircleOutlined
