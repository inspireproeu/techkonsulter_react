...
const FileUploadDialog = ({
  ...
  const handleInsert = async () => {
    ...
    try {
      ...
      for (let i = 0; i <= tempassetIDRows.length; i += 50) {
        if (tempassetIDRows.length > 0 && tempassetIDRows[i] && tempassetIDRows[i].part_no) {
          // console.log(filterassetIDRows.length,"assetIDRows", assetIDRows.length)
  ...
  const uploadFile = async () => {
    ...
    formData.append('title', `Partnumer import - ${importFiles[0].name}`);
  ...
  const handleFileUpload = (event) => {
    ...
    try {
      reader.onload = (e) => {
        ...
        data.forEach((item) => {
          if (item.status && item.status?.toUpperCase() === 'APPROVED') {
            item.status = 'published'
          }
          if (item.status && item.status?.toUpperCase() === 'NOT APPROVED') {
            item.status = 'draft'
          }
        })
