...
export const ViewDetails = (props) => {
  ...
  // console.log(columnDefinitions, "rowwww", data)
      ...
      >
        <>
        {
          columnDefinitions.map((item, i) => (
            <>
              {
                !item.hideinform ? <Row key={i} gutter={16}>
                  <Col padding={10} className={styles.gutter_row} xl={8} lg={8}	md={8} sm={24} xs={24}>
                      <div style={isMobile ? style3 : style} className={styles.labelitem}>{item.title}</div>
                  </Col>
                  <Col className={styles.gutter_row} xl={16} lg={16}	md={16} sm={24} xs={24}>
                      <div style={style1} className={styles.valueitem}>{data[item.dataIndex] ? data[item.dataIndex] : '-'}</div>
                  </Col>
                </Row> : null
              }
            </>
