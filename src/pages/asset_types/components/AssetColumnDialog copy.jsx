...
export const AssetColumnDialog = (props) => {
  ...
  useEffect(() => {
    ...
    setAllColumnsOptions(
      columnDefinitions.map((col) => ({
        ...
        checked: tempFields.indexOf('' + col.field) !== -1
      }))
    );
  ...
  return (
    ...
    >
      ...
      <ul className="sticky-toolbar1 nav flex-column pl-2 pr-2 pt-3 pb-3 mt-4">
        <li className="nav-item mb-2">
          <div>
            <List className="sidebar">
              {allColumnsOptions?.length > 0 && allColumnsOptions.map((option, index) => (
                <ListItem key={index}>
                  {
