...
export default function Step1({
    ...
    const getPartnerContact = (partner) => {
        if (partnerUsers) {
            let defaltPartnerUser = partnerUsers.find((row) => row.id === partner);
            if (defaltPartnerUser) {
                let val = {
                    partner_contact_attn: values.partner_contact_attn ? values.partner_contact_attn :  defaltPartnerUser.id
                };
                    ...
                    <Grid item xs={4}>
                        <FormControl variant="outlined" className={'formControl'}>
                            ...
                            <Autocomplete
                                ...
                                onChange={(e, option) => {
                                    if (option) {
                                        setValues({ ...values, ['partner']: option.id, ['commision_percentage']: option.commission });
                                        getPartnerusers(option.id, values)
                                    }
                                }}
