...
export default forwardRef((props, ref) => {
    const [selectedValue, setSelectedValue] = useState(props.data.date_created ? moment(props.data.date_created).format('YYYY-MM-DD') : null);
    async function handleChange(d) {
        setSelectedValue(d.target.value);
        let obj = {}
        obj.date_created = (d.target.value || null);
        obj.id = props.data.id
        delete obj.user_created
        delete obj.user_updated
        props.data.date_created= d.target.value

        fetchPut(`${DATAURLS.PROJECT.url}/${props.data.id}`, obj, getAccessToken())
            .then((response) => {
                if (response.data.id) {
                    props.setSnackBarOpen(true)
                    props.setSnackBarMessage(`Project created date has been updated for ${response.data.id}`)
