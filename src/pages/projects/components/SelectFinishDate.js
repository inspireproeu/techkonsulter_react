...
export default forwardRef((props, ref) => {
    const [selectedValue, setSelectedValue] = useState(props.data.finish_date ? moment(props.data.finish_date).format('YYYY-MM-DD') : null);

    async function handleChange(d) {
        setSelectedValue(d.target.value);
        let obj = {}
        obj.finish_date = (d.target.value || null);
        obj.id = props.data.id
        delete obj.user_created
        delete obj.user_updated
        props.data.finish_date= d.target.value

        fetchPut(`${DATAURLS.PROJECT.url}/${props.data.id}`, obj, getAccessToken())
            .then((response) => {
                if (response.data.id) {
                    props.setSnackBarOpen(true)
                    props.setSnackBarMessage(`Process finish date has been updated for ${response.data.id}`)
