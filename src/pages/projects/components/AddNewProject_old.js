...
const AddNewProjects = (props) => {
  ...
  useEffect(() => {
    if (values.client) {
      ...
      let fields = `limit=-1&sort=-id&filter={"_and":[{"email":{"_nnull":true}},{"client":{"_eq":${values.client}}}]}&fields=role_name,first_name,last_name,role.description,email,id,role.id,phone_number,userType,partner.id,client.id,access,role.name`;
  ...
  useEffect(() => {
    if (values.partner) {
      let fields = `limit=-1&sort=-id&filter={"_and":[{"email":{"_nnull":true}},{"partner":{"_eq":${values.partner}}}]}&fields=role_name,first_name,last_name,role.description,email,id,role.id,phone_number,userType,partner.id,client.id,access,role.name`;
      fetchGet(`${DATAURLS.USERS.url}?${fields}`, getAccessToken())
        .then((response) => {
