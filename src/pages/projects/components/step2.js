...
export default function Step1({
    ...
    useEffect(() => {
        if (values && values.id && values.contact_attn) {
            getClientContacts(values.contact_attn)
        }
    }, [values.contact_attn]);
    ...
    const getClient = (client, delivery_address = false) => {
        ...
        if (client) {
            ...
            let fields = `limit=-1&sort=first_name&filter={"_and":[{"status":{"_neq":"suspended"}},{"email":{"_nnull":true}},{"client":{"_eq":"${client}"}}]}&fields=isDefault,role_name,first_name,last_name,role.description,email,id,role.id,phone_number,userType,partner.id,client.id,access,role.name`;
